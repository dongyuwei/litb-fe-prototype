...
1. **JS模块化**：
	...
	4. 使用（jQuery）自定义事件在组件之间交换信息。发布的主题以`widget-xxx-topic` 形式，数据统一为单个Object Hash类型变量。`on(bind)`和`trigger`的主体均为全局window,即$(window)。
	5. 所有js代码都在`dom ready`后执行。部分特殊代码可能需要在引用处立即运行，可特殊处理。
2. **CSS模块化**：
